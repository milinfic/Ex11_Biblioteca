...
public class Tela_Pesquisa_QtdObraAutor extends javax.swing.JFrame {

    public Tela_Pesquisa_QtdObraAutor() {
        initComponents();
        combo();
    }

    public void combo() {//inciando os combos com autores, sem ter que ficar repetindo os nomes caso tenha mais de uma obra
        DefaultTableModel modelo = (DefaultTableModel) jTable1.getModel();
        modelo.setRowCount(0);

        metodos_consultar x = new metodos_consultar();
        jComboBoxAutor.addItem("selecione ...");

        ArrayList<Obra> combo = x.pesquisaautor();

        for (int i = 0; i < combo.size(); i++) {
            jComboBoxAutor.addItem(combo.get(i).getNome_autor());
        }
    }

    public void visualizar(String autor) {//classe para imprimir na tabela as obras(títulos) do autor selecionado
        metodos_consultar x = new metodos_consultar();
        DefaultTableModel modelo = (DefaultTableModel) jTable1.getModel();
        modelo.setRowCount(0);
        int qtd = 0;
        ...
        ArrayList<Obra> comboautor = x.comboAutor(autor);

        for (int i = 0; i < comboautor.size(); i++) {
            qtd++;
            modelo.addRow(new Object[]{
                comboautor.get(i).getCodigo(),
                comboautor.get(i).getArea(),
                comboautor.get(i).getEditora(),
                comboautor.get(i).getNome_obra(),
                comboautor.get(i).getNome_autor(),
                comboautor.get(i).getQtd_paginas(),});
        }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createBevelBorder(javax.swing.border.BevelBorder.RAISED), "QUANTIDADE DE OBRAS POR AUTOR", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Cambria", 0, 14))); // NOI18N
    ...
    private void jComboBoxAutorItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jComboBoxAutorItemStateChanged
        if (jComboBoxAutor.getSelectedIndex() == 0) {
            ...
            String autor;
            autor = jComboBoxAutor.getSelectedItem().toString();
            visualizar(autor);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Tela_Pesquisa_QtdObraAutor.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Tela_Pesquisa_QtdObraAutor().setVisible(true);
            }
        });
    }
